...
List* list_generate(int len) {
    List* list = list_new();
    int lentotal = 0;
    for (int j = 0; j < len - lentotal; j++) {
        int group1 = rand() % 2;
        int halfwordlen = (rand() % (1 + len - lentotal)) / 2;
        int group2 = rand() % 2;
        char* halfword = NULL;
        switch (group1) {
            case 0: //слово
                switch (group2) {
                    case 0: //палиндром
                        halfword = realloc(halfword, halfwordlen*sizeof(char));
                        for (int i = 0; i < halfwordlen; i++) {
                            halfword[i] = 'a' + (rand() % 25);
                            list_push_back(list, halfword[i]);
                            lentotal++;
                        }
                        for (int i = halfwordlen; i > 0; i--) {
                            list_push_back(list, halfword[i]);
                            lentotal++;
                        }
                        free(halfword);
